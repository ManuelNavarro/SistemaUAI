            ...
            for (var i = 0; i < $adjunct.length; i++) {
                ...
                $adjunctDiv.hover(
                    function() {
                        ...
                        var boton = "";
                         if(bontonEditar == 1){    
                            boton += ' <button class="btn btn-info btn-xs editOrg my-tooltip-link" title="Editar unidad" onClick="editarUnidadEstruct('+  $(this).children()[0].value +');">\
                                <i class="ace-icon fa fa-pencil bigger-110 icon-only"></i>\
                            </button> '
                        }
                        if(bontonEliminar == 1){
                              boton += ' <button class="btn  btn-danger btn-xs editOrg my-tooltip-link" title="Eliminar unidad" onClick="eliminarUnidadEstruct('+  $(this).children()[0].value +');">\
                    ...
                    if (opts.interactive) {
                        $nodeDiv.hover(
                            function() {
                                ...
                                var boton = "";
                                 if(bontonEditar == 1){    
                                    boton += '<button class="btn btn-info btn-xs editOrg my-tooltip-link" title="Editar unidad" onClick="editarUnidadEstruct('+  $(this).children().children()[0].value  +');">\
                                        <i class="ace-icon fa fa-pencil bigger-110 icon-only"></i>\
                                    </button> '
                                }
                                if(bontonEliminar == 1){
                                      boton += ' <button class="btn btn-purple btn-xs editOrg my-tooltip-link" title="Editar orden de unidades" onClick="ordenUnidadEstruct('+ $(this).children().children()[0].value +');">\
            ...
            $nodeDiv.hover(
                function() {
                    ...
                    if(bontonEditar == 1){    
                        ...
                        if(bontonEliminar == 1){
                              boton += ' <button class="btn  btn-danger btn-xs editOrg my-tooltip-link" title="Eliminar unidad" onClick="eliminarUnidadEstruct('+  $(this).children().children()[0].value +');">\
                                <i class="ace-icon fa fa-trash-o bigger-110 icon-only"></i>\
                            </button> '
                        }
                    if(typeof attr === typeof undefined || attr === false){
...
function mostrarInfo(id_nodo){
    jQuery.ajax({
        ...
        success: function(json){
            ...
            var area = "";
            var descripcion = "N/D";
            var centro_costo = "N/D";

            if(json['unidad'][0]['centro_costo_id']){
                centro_costo = json['unidad'][0]['centro_costo_id'];
            }

            if(json['unidad'][0]['descripcion']){
                descripcion = json['unidad'][0]['descripcion'];
            }

            if(json['empleado'][0]){
                
                if(json['persona'][0]){
                    empleado =json['persona'][1]['codigo_empleado'] + " - " + json['persona'][0]['nombre'] +" "+ json['persona'][0]['apellido'];
...
function ordenUnidadEstruct(id_nodo){
    jQuery.ajax({
...
function editarUnidadEstruct(id_nodo){
    estructura= $('#estructuraorganizativatable').DataTable().rows('.selected').data()[0][0];
    //editarUnidadEstruct
    jQuery.ajax({
        url: "/organizacion/redirectUnidad/",
        type: "POST",
        processData: false,
        data: "csrfmiddlewaretoken="+$("[name=csrfmiddlewaretoken]").val()+"&unidad_id="+id_nodo+"&estructura_organizativa_id="+estructura,
        async: false,
        success: function(json){
            
          //  parent.location="/organizacion/unidad"
...
function eliminarUnidadEstruct(id_nodo, accion){
    bootbox.dialog({
        ...
        buttons: {
            Aceptar: {
                ...
                callback: function() {
                    ...
                    type: "POST",
                    processData: false,
                    data: "csrfmiddlewaretoken="+$("[name=csrfmiddlewaretoken]").val()+"&accion=eliminarUnidadEstructura&id_nodo="+id_nodo,
                    success: function(json){
                        if(json[0] && json[0]['bandera']){
                            id_seleccionado= $('#estructuraorganizativatable').DataTable().rows('.selected').data()[0][0];
                            jQuery.ajax({
                                url: "/organizacion/org/",
                                type: "POST",
                                processData: false,
                                data: "csrfmiddlewaretoken="+
                                                        $("[name=csrfmiddlewaretoken]").val()+"&accion=obtenerEstructuraOrganizativa&id_seleccionado="+id_seleccionado,
                                success: function(html){
                                    document.getElementById("basic-chart-source").innerHTML = html;
