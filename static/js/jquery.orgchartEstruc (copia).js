            ...
            for (var i = 0; i < $adjunct.length; i++) {
                ...
                $adjunctDiv.hover(
                    function() {
                        ...
                        if($('#estructuraorganizativatable').DataTable().rows('.selected').data()[0][5] == "Nuevo(a)"){
                            $(this).attr('data-content','<div class="widget-main no-padding">\
                                                            <p>\
                                                                <button class="btn btn-success btn-xs"  onClick="mostrarInfo('+ $(this).children()[0].value +');">\
                                                                    <i class="ace-icon fa fa-external-link bigger-110 icon-only"></i>\
                                                                </button>\
                                                                <button class="btn btn-info btn-xs" onClick="editarUnidad('+ $(this).children()[0].value +');">\
                                                                    <i class="ace-icon fa fa-pencil bigger-110 icon-only"></i>\
                                                                </button>\
                                                                <button class="btn btn-danger btn-xs" onClick="eliminarUnidad('+ $(this).children()[0].value +');">\
                        ...
                        $nodeDiv.hover(
                            function() {
                                ...
                                if($('#estructuraorganizativatable').DataTable().rows('.selected').data()[0][5] == "Nuevo(a)"){
                                    $(this).attr('data-content','<div class="widget-main no-padding">\
                                                                    <p>\
                                                                        <button class="btn btn-success btn-xs"  onClick="mostrarInfo('+ $(this).children().children()[0].value +');">\
                                                                            <i class="ace-icon fa fa-external-link bigger-110 icon-only"></i>\
                                                                        </button>\
                                                                        <button class="btn btn-info btn-xs" onClick="editarUnidad('+ $(this).children().children()[0].value +');">\
                                                                            <i class="ace-icon fa fa-pencil bigger-110 icon-only"></i>\
                                                                        </button>\
                                                                        <button class="btn btn-purple btn-xs" onClick="ordenUnidad('+ $(this).children().children()[0].value +');">\
                    ...
                    if($('#estructuraorganizativatable').DataTable().rows('.selected').data()[0][5] == "Nuevo(a)"){
                        $(this).attr('data-content','<div class="widget-main no-padding">\
                                                        <p>\
                                                            ...
                                                            <button class="btn btn-danger btn-xs" onClick = "eliminarUnidad('+ $(this).children().children()[0].value +');">\
                                                                <i class="ace-icon fa fa-trash-o bigger-110 icon-only"></i>\
                                                            </button>\
                                                        </p>\
                                                        <div class="center">\
...
function mostrarInfo(id_nodo){
    jQuery.ajax({
        ...
        success: function(json){
            ...
            if(json['empleado'][0]){
                empleado = "<b>Nombre completo: </b>" + json['empleado'][0]['nbr_empleado'] +" "+ json['empleado'][0]['ap_empleado'];
                if(json['empleado'][0]['telefono']){
                    empleado += "<b> Teléfono: </b>"+ json['empleado'][0]['telefono'];
                }
...
function editarUnidad(id_nodo){
    ...
    jQuery.ajax({
        ...
        type: "POST",
        processData: false,
        data: "csrfmiddlewaretoken="+$("[name=csrfmiddlewaretoken]").val()+"&accion=editUnidad&id_nodo="+id_nodo+"&id_estructura="+estructura,
        success: function(json){
           // alert(json[0]['bandera'])
        ...
        success: function(json){
            if(json[0] && json[0]['bandera']){
                ...
                jQuery.ajax({
                 ...
                 create_alert("<li>Unidad eliminada de la relación entre unidades</li>","EXITO","notificaciones",true,6000);
            }else{
                 create_alert("<li>Error en la eliminación de la relación de unidad</li>","ERROR","notificaciones",true,6000);
            }
        }
        });
    }else{
        bootbox.alert("La operación ha sido cancelada", function() {
        });
    }
}); 
